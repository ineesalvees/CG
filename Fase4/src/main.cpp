...
Group* groupReader(pugi::xml_node group,Translation *translation, Rotation *rotation,Color *color, Scale *scale,Light *light) {
    ...
    for (pugi::xml_node attr = group.first_child(); attr; attr = attr.next_sibling()) //percorre os varios atributos
    {
        ...
        if (strcmp(attr.name(),"lights")==0) {

            for (pugi::xml_node light = attr.first_child(); light; light = light.next_sibling())
            {
                pugi::xml_attribute x , y , z;
                std::string xv,yv,zv;


                pugi::xml_attribute type = light.first_attribute();
                x = type.next_attribute();
                y = x.next_attribute();
                z = y.next_attribute();
                xv = x.value();
                yv = y.value();
